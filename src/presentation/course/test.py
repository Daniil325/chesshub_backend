...
async def post_article(test: CreateTest, cmd: FromDishka[CreateTestCommand]):
...
async def update_article(
    article: UpdateTest, cmd: FromDishka[UpdateTestCommand], id: UUID = Path(...)
):
    await cmd(UpdateTestDto(article_id=id, **article))
    return SuccessResponse()


@router.delete("/{id}", response_model=SuccessResponse)
async def delete_article(cmd: FromDishka[DeleteTestCommand], id: UUID = Path(...)):
    await cmd(id)
    return SuccessResponse()
